  ...
  <div class="table">
    ...
    <div class="news_list" v-for="n in new_list.length">
      ...
      <div class="bottom clearfix">
        <a class="news_media right" v-bind:href="computeSource(new_list[n-1].source_media)" target="_blank">来源:{{new_list[n-1].source_media}}</a>
        <div class="news_author right">作者:{{new_list[n-1].author}}</div>
...
export default {
  ...
  created: function(){
    ...
    GetPage(page_idx){
      var size = 10;
      var url = server_config.url+"/back/getnewsbycoinid.php?&offset="+(page_idx*size)+"&size="+size+"&coin_id="+this.$props.id;
      axios.get(
        url,
        {
        method:'get',
