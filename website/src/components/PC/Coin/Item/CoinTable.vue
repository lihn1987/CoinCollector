...
export default {
  ...
  created: function(){
    ...
    pageChange(pInfo){
      ...
      var url = "http://"+server_config.url+":"+server_config.port+"/back/getcoinbase.php?method=get_all&page_idx="+((pInfo.pageNumber-1)*pInfo.pageSize)+"&page_size="+pInfo.pageSize;
      axios.get(
        url,
        {
        method:'get',
