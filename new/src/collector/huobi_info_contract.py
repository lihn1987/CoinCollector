...
class myThread (threading.Thread):
    ...
    def connect(self):
        if config["proxy_config"]["proxy_use"]:
            self.huobi_ws.connect("wss://api.hbdm.com/ws", http_proxy_host=config["proxy_config"]["proxy_ip"], http_proxy_port=config["proxy_config"]["proxy_port"])
        else:
            self.huobi_ws.connect("wss://api.hbdm.com/linear-swap-ws")
    ...
    def on_huobi_recv(self, str):
        ...
        if 'ping' in json_data:
            ...
            #保存到redis
            k = order_coin+"-"+base_coin+"-HUOBI_CONTRACT-DEPTH"
    ...
    def sub_depth(self):
        ...
        for item in self.coin_list:
            ...
            self.send_data({
            "sub": "market.%s.depth.step0"%symbel.upper(),
            "id": "asefddfeasdfefgh"
            })
...
if __name__ == "__main__":
    ...
    # 若没设置，初始化只看BTC和ETH
    if redis_db.get("HUOBI_CONTRACT-CONFIG") == None or redis_db.get("OK-CONFIG") == None:
        redis_db.set("HUOBI_CONTRACT-CONFIG", '[["BTC", "USDT"], ["ETH", "USDT"]]')
        redis_db.set("OK-CONFIG", '["BTC-USDT", "ETH-USDT"]')

    process_list = []
    coin_list = json.loads(redis_db.get("HUOBI_CONTRACT-CONFIG"))
    restart_processes(process_list, coin_list)
    
    print("开始订阅监听币种的信息")
    ps = redis_db.pubsub()
    ps.subscribe('HUOBI_CONTRACT-CONFIG') 
    next(ps.listen())
