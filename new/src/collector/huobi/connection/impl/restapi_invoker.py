...
def call_sync(request, is_checked=False):
    if request.method == "GET":
        ...
        response = session.get(request.host + request.url, headers=request.header, timeout=10)
        ...
        response = session.post(request.host + request.url, data=json.dumps(request.post_body), headers=request.header, timeout=10)
        dict_data = json.loads(response.text, encoding="utf-8")
        # print("call_sync  === recv data : ", dict_data)
        check_response(dict_data)
        return request.json_parser(dict_data)
