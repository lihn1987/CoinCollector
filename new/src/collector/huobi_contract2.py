...
def WatchDog():
    redis_db.set("%s-%s-latest_time"%(order_coin,base_coin), time.time())
...
def GetSellPriceAndCount():
    ...
    print(trade_list)
    print("GetSellPriceAndCount", (DIR_BUY if trade_dir == DIR_SELL else DIR_SELL, sum_money/sum_count*(0.9896 if trade_dir == DIR_SELL else 1.0104), sum_count))
    price = None
    """
...
def GetCountPersent(price, dir):
    ...
    if dir == DIR_BUY:
        rtn = [
            int(amount_usdt_all*persent_list[0]/10/price),
            int(amount_usdt_all*persent_list[1]/10/price)
            ]
    elif dir == DIR_SELL:
        rtn = [
            int(amount_usdt_all*persent_list[0]/10/price),
...
def GetPersentMaker(price, dir):
    ...
    if dir == DIR_BUY:
        rtn = [
            [dir, (1-step_list[0])*price, int(amount_usdt_all*persent_list[0]/10/price), 0],
            [dir, (1-step_list[1])*price, int(amount_usdt_all*persent_list[1]/10/price), 0],
            ]
    elif dir == DIR_SELL:
        rtn = [
            [dir, (1+step_list[0])*price, int(amount_usdt_all*persent_list[0]/10/price), 0],
...
def GetProfitRate():
    ...
    try:
        page_size = 0
        args = {
            "contract_code":"%s-USDT"%(order_coin,)
        }
        res = Post("/linear-swap-api/v1/swap_cross_position_info", args, False)
        if res["status"] == "ok" and len(res["data"]) != 0:
            print("当前盈利状态为",res["data"][0]["profit_rate"])
            print(res["data"][0]["volume"], res["data"][0]["profit_rate"])
            return (res["data"][0]["volume"], res["data"][0]["profit_rate"])
        else:
            print("已经卖掉了，不用算盈利啦")
            return (0, 0)
...
def CloseMaker(volume):
    print("开始闪电平仓")
    ClearMaker()
    args = {
            "contract_code":"%s-USDT"%(order_coin,),
            "volume":int(volume),
            "direction": "sell" if trade_dir == DIR_BUY else "buy"
        }
    print(args)
    Post( "/linear-swap-api/v1/swap_cross_lightning_close_position", args, True)
    ...
    while True:
        ...
        if event_flag:
            ...
            if trade_dir != dir_tmp:
                ...
                SyncMaker()
            SaveEnv()
        elif GetProfitRate()[1] < -0.35:
            print("止损！！！！！！！！！！！！！！！！！！！！！！！！！！！！")
            CloseMaker(GetProfitRate()[0])
            break
        else:
            time.sleep(1)
