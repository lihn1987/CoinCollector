...
def Buy(redis_db, order_coin, base_coin, amount):
    print("buy", order_coin, base_coin, amount)
    trade_client = TradeClient(api_key=key.ak,
                                secret_key=key.sk)
    precision = int(redis_db.get("%s_%s-HUOBI-value-precision"%(order_coin, base_coin)))
    amount = float(format(amount, '.%df'%(precision)))
    while True:
        try:
...
def Sell(redis_db, order_coin, base_coin, amount):
    print("Sell", order_coin, base_coin, amount)
    trade_client = TradeClient(api_key=key.ak,
                                secret_key=key.sk)
    precision = int(redis_db.get("%s_%s-HUOBI-amount-precision"%(order_coin, base_coin)))
    amount = float(format(amount, '.%df'%(precision)))
    while True:
        #try:
        order_id = trade_client.create_order(
...
def ThreadFunc(order_coin, base_coin, market, period):
    ...
    while True:
        ...
        max_profit = float(redis_db.get("HAIGUI-%s-%s-%s-%s-max_profit"%(order_coin, base_coin, market, period)))
        ...
        if is_buy == False:
            ...
            for item in kline_list:
                ...
                if not _min or item["low"] < _min:
                    _min = item["low"]
            redis_db.delete("HAIGUI-%s-%s-%s-%s-profit"%(order_coin, base_coin, market, period))
            redis_db.delete("HAIGUI-%s-%s-%s-%s-price"%(order_coin, base_coin, market, period))
            if max_profit > 101 and (_max - _min)*100/_min > best_buy_point and _max == kline_list[0]["high"]:
                is_buy = True
                result = Buy(redis_db, order_coin, base_coin, amount)
        ...
        if is_buy == True:
            ...
            for item in kline_list:
                ...
                if not _min or item["low"] < _min:
                    _min = item["low"]
            redis_db.set("HAIGUI-%s-%s-%s-%s-profit"%(order_coin, base_coin, market, period), (kline_list[0]["high"] - price)*100/price )
            if (_max - price)*100/price > best_sell_point_high and _max == kline_list[0]["high"] or (price - kline_list[0]["low"])*100/price > best_sell_point_low and _min == kline_list[0]["low"]:
                is_buy = False
                result = Sell(redis_db, order_coin, base_coin, amount)
                amount = int((result[0] - result[1])*10000)/10000
                redis_db.set("HAIGUI-%s-%s-%s-%s-is_buy"%(order_coin, base_coin, market, period), 1 if is_buy else 0)
                redis_db.set("HAIGUI-%s-%s-%s-%s-amount"%(order_coin, base_coin, market, period), amount)
        #print(kline_list)
...
if __name__ == "__main__":
    ...
    _thread.start_new_thread ( ThreadFunc, ("eth3s", "usdt", "huobi", "1min"))
    _thread.start_new_thread ( ThreadFunc, ("eth3l", "usdt", "huobi", "1min"))
    _thread.start_new_thread ( ThreadFunc, ("link3s", "usdt", "huobi", "1min"))
    _thread.start_new_thread ( ThreadFunc, ("link3l", "usdt", "huobi", "1min"))
    _thread.start_new_thread ( ThreadFunc, ("algo", "usdt", "huobi", "1min"))
    while True:
        time.sleep(1)
